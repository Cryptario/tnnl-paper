...
\begin{itemize}[noitemsep,topsep=0pt]
	...
	\item \textbf{Balance} --- $ v_{pub}^{old} + \sum_{i=0}^{1} n_{i}^{old}.v = v_{pub}^{new} + \sum_{i=0}^{1} n_{i}^{new}.v \in \{0 .. 2^{64} - 1\}$.
	\item \textbf{Nullifier integrity} --- for each $i \in \{0, 1\}: \nf_{i}^{old} = \func{compNull}{n_i}{\aski{}_i} $.% (where \func{compNull} is as described in \cref{sec:nullifs}).
	\item \textbf{Spend authority} --- for each $i \in \{0, 1\}: n_i^{old}.\apkT = \func{derivePubAccountKey}{\aski{}_i}$.
	\item \textbf{Non-malleability} --- for each $i \in \{0, 1\}: h_{i} = \func{compAuth}{i}{\aski{}_i}{h_{sig}} $ (where \func{compAuth}{...}\xspace is as described in \cref{sec:authCodes}).
	\item \textbf{Uniqueness of $\vec{\rho}^{new}$} --- for each $i \in \{0, 1\}: n_i.\rho^{new} = \func{derRho}{i}{\varphi}{h_{sig}} $ (where \func{derRho}{...}\xspace is as described in \cref{sec:derRho}).
	\item \textbf{Note commitment integrity} --- for each $i \in \{0, 1\}: \varsigma = \func{commit}{n_i}.$
\end{itemize}
